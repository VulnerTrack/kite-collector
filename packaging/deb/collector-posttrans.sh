...
# removal — the point where a shell-cached path or a self-registered /etc
# unit's ExecStart is actually dangling — so the same bridge runs again
# here. Keep the bridge logic in sync with collector-postinstall.sh.
set -e
...
if [ -x "$newbin" ] && [ ! -e "$legacy" ] && [ ! -L "$legacy" ]; then
    ...
    if [ -f "$unit" ] && grep -q "$legacy" "$unit" && [ -d /run/systemd/system ]; then
