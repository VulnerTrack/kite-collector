...
const (
	CollectorService_ReportMachines_FullMethodName   = "/kite.v1.CollectorService/ReportMachines"
...
type CollectorServiceClient interface {
	// ReportMachines streams machine snapshots from agents to the server.
	ReportMachines(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[MachineSnapshot, ReportResponse], error)
...
func (c *collectorServiceClient) ReportMachines(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[MachineSnapshot, ReportResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &CollectorService_ServiceDesc.Streams[0], CollectorService_ReportMachines_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[MachineSnapshot, ReportResponse]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type CollectorService_ReportMachinesClient = grpc.ClientStreamingClient[MachineSnapshot, ReportResponse]
...
type CollectorServiceServer interface {
	// ReportMachines streams machine snapshots from agents to the server.
	ReportMachines(grpc.ClientStreamingServer[MachineSnapshot, ReportResponse]) error
	// ReportFindings streams configuration audit findings from agents.
...
func (UnimplementedCollectorServiceServer) ReportMachines(grpc.ClientStreamingServer[MachineSnapshot, ReportResponse]) error {
	return status.Error(codes.Unimplemented, "method ReportMachines not implemented")
}
...
func _CollectorService_ReportMachines_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CollectorServiceServer).ReportMachines(&grpc.GenericServerStream[MachineSnapshot, ReportResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type CollectorService_ReportMachinesServer = grpc.ClientStreamingServer[MachineSnapshot, ReportResponse]
...
var CollectorService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ReportMachines",
			Handler:       _CollectorService_ReportMachines_Handler,
			ClientStreams: true,
		},
		{
