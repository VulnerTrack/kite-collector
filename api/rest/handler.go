...
func (h *Handler) Mux() *http.ServeMux {
	...
	mux.HandleFunc("POST /api/v1/scans/{id}/cancel", h.handleCancelScan)
...
func (h *Handler) handleListEvents(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("events List query failed",
			"code", string(LogCodeEventsList),
			"error", err,
			"event_type_filter", filter.EventType,
			"limit", filter.Limit,
			"offset", filter.Offset)
		writeError(w, http.StatusInternalServerError, "internal error")
		return
...
func (h *Handler) handleListScans(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("scans GetLatest query failed",
			"code", string(LogCodeScansGetLatest),
			"error", err,
			"path", r.URL.Path)
		writeError(w, http.StatusInternalServerError, "internal error")
		return
...
func (h *Handler) handleListIncidents(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("runtime_incidents List query failed",
			"code", string(LogCodeRuntimeIncidentsList),
			"error", err,
			"incident_type_filter", filter.IncidentType,
			"limit", filter.Limit,
			"offset", filter.Offset)
		writeError(w, http.StatusInternalServerError, "internal error")
		return
...
func (h *Handler) handleListNetworkScanEvents(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("network_scan_events List query failed",
			"code", string(LogCodeNetworkScanEventsList),
			"error", err,
			"limit", filter.Limit,
			"offset", filter.Offset)
		writeError(w, http.StatusInternalServerError, "internal error")
...
func (h *Handler) handleListNetworkOpenPorts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("network_open_ports List query failed",
			"code", string(LogCodeNetworkOpenPortsList),
			"error", err,
			"scan_id_filter", filter.ScanID,
			"limit", filter.Limit,
			"offset", filter.Offset)
		writeError(w, http.StatusInternalServerError, "internal error")
		return
...
func (h *Handler) handleListSafetyGuardEvents(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("safety_guard_events List query failed",
			"code", string(LogCodeSafetyGuardEventsList),
			"error", err,
			"guard_type_filter", filter.GuardType,
			"limit", filter.Limit,
			"offset", filter.Offset)
		writeError(w, http.StatusInternalServerError, "internal error")
		return
