...
VulnerTrack only collects the minimum data required to support its declared use cases. Every field in the machine model must have a documented purpose and a clear mapping to at least one supported use case (CSAM, compliance, lifecycle, exposure). Fields without documented purpose must not be collected or stored.[^9][^10]

**Binding rule:** The full list of fields collected per machine type must be documented in `docs/architecture.md` and kept current. Users must be able to inspect everything VulnerTrack collects by querying the local SQLite file or the API.
...
VulnerTrack uses a **push model** for machine and exposure events, aligned with the OpenTelemetry push architecture. The agent/CLI generates events when state changes are detected and pushes them to the configured endpoint. The platform (API or OTel Collector) never polls individual agents.[^1][^13][^14]

Rationale: machine discovery events are irregular, security-relevant facts, not periodic metrics. Push better supports real-time detection of unauthorized machines. Additionally, agents frequently run behind NAT, firewalls, or in cloud environments where inbound connections are restricted, making pull impractical.[^15][^16]
### 2.2 Pull Model Only for Aggregate KPIs (Prometheus)
The Prometheus `/metrics` endpoint on the VulnerTrack API is a **secondary, optional surface** for aggregate KPIs only. It does not expose individual machine records or raw events. Its sole purpose is to allow teams to integrate VulnerTrack health and CSAM KPIs into existing Grafana/Prometheus dashboards without additional integration work.[^17][^18][^19]

**Binding rule:** Raw machine records and events must never be exposed via the Prometheus endpoint. Only counters, gauges, and summaries aggregated at the inventory level are permitted.
...
***
## 3. Machine Classification Guidelines
### 3.1 Authorized vs Unauthorized
The `is_authorized` field is the most security-critical field in the machine model. It must be derived by matching discovered machines against at least one configured "source of truth" (CMDB, Active Directory, MDM, cloud account inventory, or an explicit allowlist). A missing or empty source-of-truth configuration must result in `is_authorized: unknown`, never `is_authorized: true`.[^9][^22]

**Binding rule:** VulnerTrack must never default to marking machines as authorized. The default posture is `unknown`; `true` requires a positive match.[^23][^9]
### 3.2 Managed vs Unmanaged
The `is_managed` field reflects whether an machine has the required security controls installed and reporting. Required controls are configurable per environment (e.g., EDR agent, MDM enrollment, disk encryption). An machine is `is_managed: false` if any required control is missing or not recently reporting.[^9][^24][^10]
...
`UnauthorizedMachineDetected` and `UnmanagedMachineDetected` events carry a `severity` field. Severity must be derived from configurable rules (e.g., unauthorized machine in a `prod` environment → high; in `dev` → low). Default severity rules must be documented and must err on the side of higher severity in the absence of environment context.[^9][^25][^10]
...
- **Agent-based discovery:** A lightweight agent that runs on an endpoint and pushes machine metadata on a schedule.
...
### 5.1 Machine Identity and Deduplication
Every discovered machine must be assigned a stable `machine_id` (UUID) that persists across scan runs. Deduplication logic must use a configurable set of identifiers (hostname, MAC, cloud instance ID) to match new observations to existing records. Deduplication rules must be documented and testable.[^8][^9]
### 5.2 `last_seen` Freshness
Every machine record must carry a `last_seen` timestamp that reflects the most recent confirmed observation. Machines that exceed a configurable `stale_threshold` must be flagged and optionally emit an `MachineNotSeen` event. Stale data must be clearly indicated in reports and API responses.[^8][^28]
...
|---|---|
| NIST SP 1800-5[^8][^29] | Machine model must cover all NIST-recommended attributes; lifecycle states must be supported |
| CIS Control 1.1[^22][^23] | Scan output must be exportable as a detailed machine inventory with all required fields |
| CIS Control 1.2[^22][^23] | `UnauthorizedMachineDetected` events must be generated within one scan cycle of detection; remediation workflows must be documentable |
| ISO 27001 A.8[^27][^30] | `owner`, `machine_type`, `lifecycle_state`, and classification fields must be present and queryable |
...
VulnerTrack integrations (connectors, exporters, webhooks) must not modify the core machine model or event schema. They consume data from VulnerTrack and produce output for external systems, but the canonical machine record always lives in VulnerTrack.[^1][^13][^20]
### 8.2 Supported Integration Surfaces (v1)
- **SQLite file** — direct SQL queries for scripting and automation
- **REST API** — machine queries, event queries, inventory exports
...
│   │
│   ├── model/              # Machine, Event, and Policy data models (Go structs)
│   ├── classifier/         # Authorization and management classification logic
│   ├── dedup/              # Machine deduplication and identity resolution
│   ├── store/
...
│   ├── rest/               # REST handlers (machine queries, exports, events)
...
[Event Emitter]
  ├── MachineDiscovered
  ├── MachineUpdated
  ├── UnauthorizedMachineDetected
  ├── UnmanagedMachineDetected
  └── MachineNotSeen
      │
      ▼
[OTLP HTTP/gRPC push]
...
[VulnerTrack API]
  ...
  ├── PostgreSQL (machine graph + event log)
  ├── REST query API
...
[ ] New fields in machine model have documented purpose
...
|---|---|
| Defaulting machines to `is_authorized: true` | Creates false sense of security; violates CIS 1.2[^22][^23] |
