...
// TestLogCodes_FollowConvention pins the `discovery.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeRegistrySourceCircuitOpen,
		LogCodeRegistrySourceFailed,
		LogCodeHeartbeatRecordFailed,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "discovery", parts[0],
				"code %q must lead with the discovery namespace prefix", s)
