...
// TestLogCodes_FollowConvention pins the `entra.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		...
		LogCodeEnrichMfaReportUnavailable,
		LogCodePaginationMaxObjectsTripped,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "entra", parts[0],
				"code %q must lead with the entra namespace prefix", s)
