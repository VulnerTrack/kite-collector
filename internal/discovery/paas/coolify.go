...
func (c *Coolify) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	slog.Info("Coolify PaaS discovery starting", //#nosec G706 -- control chars sanitized; operator-configured env var
		"code", string(LogCodeCoolifyStarting),
		"endpoint", sanitizeLogValue(endpoint))
	...
	if err := client.get(ctx, "/api/v1/servers", &servers); err != nil {
		slog.Warn("Coolify list-servers failed; continuing with apps-only discovery",
			"code", string(LogCodeCoolifyListServersFailed),
			"error", err,
			"apps_found", len(apps))
	} else {
		for i := range servers {
