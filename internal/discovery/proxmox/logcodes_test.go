...
// TestLogCodes_FollowConvention pins the `proxmox.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeEnumerateVMsFailed,
		LogCodeEnumerateLXCFailed,
		LogCodeEnumerateVMConfigFailed,
		LogCodeEnumerateSnapshotsFailed,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "proxmox", parts[0],
				"code %q must lead with the proxmox namespace prefix", s)
