...
// TestLogCodes_FollowConvention pins the `docker.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeEnumerateEnvInspectFailed,
		LogCodeEnumerateInspectFailed,
		LogCodeEnumerateListImagesFailed,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "docker", parts[0],
				"code %q must lead with the docker namespace prefix", s)
