...
// TestLogCodes_FollowConvention pins the `unifi.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeCloudListDevicesFailed,
		LogCodeLocalListClientsFailed,
		LogCodeLocalListDevicesFailed,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "unifi", parts[0],
				"code %q must lead with the unifi namespace prefix", s)
