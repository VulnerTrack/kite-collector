...
func (o *OVHcloud) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	slog.Info("OVHcloud VPS discovery starting", //#nosec G706 -- control chars sanitized; operator-configured env var
		"code", string(LogCodeOVHCloudStarting),
		"region", sanitizeLogValue(region))
...
func (o *OVHcloud) discoverDedicated(ctx context.Context, client *apiClient, base string, auth authFunc) ([]model.Asset, error) {
	...
	for _, name := range names {
		...
		if sErr != nil {
			slog.Warn("OVHcloud skipping dedicated server with unsafe name",
				"code", string(LogCodeOVHCloudSkipUnsafeDedicated),
		...
		if err := client.get(ctx, "/dedicated/server/"+safeName, &srv); err != nil {
			slog.Warn("OVHcloud get-dedicated-server API call failed",
				"code", string(LogCodeOVHCloudGetDedicatedFailed),
				"name", safeName,
				"error", err)
			continue
		}
...
func (o *OVHcloud) discoverVPS(ctx context.Context, client *apiClient, base string, auth authFunc) ([]model.Asset, error) {
	...
	for _, name := range names {
		...
		safeName, sErr := safenet.SanitizePathSegment(name)
		if sErr != nil {
			slog.Warn("OVHcloud skipping VPS with unsafe name",
				"code", string(LogCodeOVHCloudSkipUnsafeVPS),
				"name", sanitizeLogValue(name),
				"error", sErr)
			continue
		}
		...
		if err := client.get(ctx, "/vps/"+safeName, &instance); err != nil {
			slog.Warn("OVHcloud get-VPS API call failed",
				"code", string(LogCodeOVHCloudGetVPSFailed),
				"name", safeName,
				"error", err)
			continue
		}
