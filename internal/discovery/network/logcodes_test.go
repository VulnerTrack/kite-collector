...
// TestLogCodes_FollowConvention pins the `network.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeScannerStarting,
		LogCodeScannerSafetyGuardFired,
		LogCodeScannerGuardEventPersistFail,
		LogCodeScannerScanEventPersistFail,
		LogCodeScannerOpenPortsPersistFail,
		LogCodeScannerInvalidCIDRSkipped,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "network", parts[0],
				"code %q must lead with the network namespace prefix", s)
