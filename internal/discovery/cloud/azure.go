...
func (az *Azure) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	if subscriptionID != "" {
		...
		slog.Info("Azure VM enumerating accessible subscriptions",
			"code", string(LogCodeAzureVMEnumeratingSubs),
			"tenant_id", creds.tenantID)
		...
		if len(enumerated) == 0 {
			slog.Warn("Azure VM no accessible subscriptions; skipping discovery",
				"code", string(LogCodeAzureVMNoSubsAccessible),
				"tenant_id", creds.tenantID)
			return nil, nil
		}
		subscriptionIDs = enumerated
		slog.Info("Azure VM subscriptions discovered",
			"code", string(LogCodeAzureVMSubsDiscovered),
	...
	for _, subID := range subscriptionIDs {
		...
		slog.Info("Azure VM listing VMs for subscription",
			"code", string(LogCodeAzureVMListingVMs),
...
func (az *Azure) fetchVMPage(ctx context.Context, apiURL, token string) ([]azureVM, string, error) {
	...
	for _, raw := range result.Value {
		...
		if vmErr != nil {
			slog.Debug("Azure VM skipping unparseable entry from API response",
				"code", string(LogCodeAzureVMSkipUnparseable),
				"error", vmErr)
			continue
		}
		vms = append(vms, vm)
