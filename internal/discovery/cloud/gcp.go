...
func obtainGCPToken(ctx context.Context) (string, error) {
	...
	if err == nil && token != "" {
		slog.Info("GCP Compute token obtained from GCE metadata server",
			"code", string(LogCodeGCPComputeTokenFromMetadata))
		return token, nil
	}
...
func (g *GCP) fetchDiskSourceImage(ctx context.Context, diskURL, token string) string {
	...
	if err != nil {
		slog.Debug("GCP Compute disk resource fetch failed; OS enrichment skipped",
			"code", string(LogCodeGCPComputeDiskFetchFailed),
			"error", err,
			"url", diskURL)
		return ""
	}
