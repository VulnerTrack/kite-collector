...
func (s *Source) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	if len(ifaces) == 0 {
		slog.Info("WS-Discovery no multicast-capable interfaces selected; discovery skipped",
			"code", string(LogCodeWSDNoInterfaces))
		return nil, nil
	}
	...
	for _, iface := range ifaces {
		if !parsed.DisableIPv4 {
			if conn, err := listenMulticast(iface, wsdIPv4); err == nil {
				...
				slog.Debug("WS-Discovery IPv4 multicast listen failed on interface; skipping leg",
					"code", string(LogCodeWSDIPv4ListenFail),
					"iface", iface.Name,
					"error", err)
			}
		}
		if !parsed.DisableIPv6 {
			if conn, err := listenMulticast(iface, wsdIPv6); err == nil {
				...
				slog.Debug("WS-Discovery IPv6 multicast listen failed on interface; skipping leg",
					"code", string(LogCodeWSDIPv6ListenFail),
					"iface", iface.Name,
					"error", err)
			}
		}
...
func sendAndReadReplies(ctx context.Context, iface net.Interface, group net.IP, types string, record func(probeMatch, net.IP)) {
	...
	if err != nil {
		slog.Debug("WS-Discovery sender socket open failed; skipping Probe on interface",
			"code", string(LogCodeWSDOpenSenderFail),
			"iface", iface.Name,
			"group", group.String(),
			"error", err)
		return
	...
	if _, err := conn.WriteTo(payload, dst); err != nil {
		slog.Debug("WS-Discovery Probe write to multicast group failed",
			"code", string(LogCodeWSDSendFail),
			"iface", iface.Name,
			"group", group.String(),
			"error", err)
		return
