...
func (s *Source) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	if len(dests) == 0 {
		slog.Info("netbios: no broadcast targets and no explicit targets; skipping")
		return nil, nil
	}
	...
	if err := enableBroadcast(conn); err != nil {
		...
		slog.Debug("netbios: SO_BROADCAST not set",
			"error", err)
	}
	...
	for _, dst := range dests {
		if _, err := conn.WriteTo(payload, &net.UDPAddr{IP: dst, Port: nbnsPort}); err != nil {
			slog.Debug("netbios: send failed", "dst", dst.String(), "error", err)
		}
	}
