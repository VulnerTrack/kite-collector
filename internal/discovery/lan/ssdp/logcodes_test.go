...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeSSDPNoInterfaces, LogCodeSSDPIPv4ListenFail, LogCodeSSDPIPv6ListenFail,
		LogCodeSSDPOpenSenderFail, LogCodeSSDPSendFail,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "lan_ssdp", parts[0],
				"code %q must lead with the lan_ssdp namespace prefix", s)
