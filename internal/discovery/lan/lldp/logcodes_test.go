...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeLLDPCtlMissing, LogCodeLLDPCtlNonZero, LogCodeLLDPSkipMalformedIface,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "lan_lldp", parts[0],
				"code %q must lead with the lan_lldp namespace prefix", s)
