...
func (s *Source) Discover(ctx context.Context, cfg map[string]any) ([]model.Asset, error) {
	...
	if err != nil {
		slog.Info("lldpctl binary not on PATH; LLDP discovery skipped (install lldpd to enable)",
			"code", string(LogCodeLLDPCtlMissing),
...
func parseLLDPCtl(raw []byte) ([]neighbor, error) {
	...
	for _, e := range entries {
		...
		if err := absorbInterface(&n, e.value); err != nil {
			slog.Debug("LLDP interface entry malformed; skipping",
				"code", string(LogCodeLLDPSkipMalformedIface),
				"iface", e.key,
				"error", err)
			continue
		}
