...
// fileCollector walks Primary install roots + per-user dirs.
...
func (c *fileCollector) Name() string { return "winargprimary" }

func (c *fileCollector) Collect(_ context.Context) ([]Row, error) {
	out := make([]Row, 0, 16)

	roots := append([]string{}, c.installRoots...)
	if p := strings.TrimSpace(c.getenv("PRIMARY_DIR")); p != "" {
	...
	for _, base := range c.usersBases {
		...
		for _, e := range entries {
			...
			for _, rel := range UserPrimaryDirs() {
...
func (c *fileCollector) consider(path, user string, out *[]Row) {
	...
	row := Row{
		...
		AccountCuentaSuffix4: AccountSuffix4(filepath.Base(path)),
	}
	...
	skipBody := ext == ".msi" || ext == ".exe" || ext == ".parquet"
...
func (c *fileCollector) mergeFields(row *Row, body []byte) {
	...
	if row.Environment == EnvProduction {
		row.HasProductionEndpoint = true
	}
	row.BrokerRoute = BrokerRouteFromBody(body)
	...
	case KindCredentialsJSON, KindPyRofexConfig:
		fields = ParsePrimaryConfig(body)
	case KindTokenCache:
		fields = ParsePrimaryTokenCache(body)
	case KindOrderAudit:
		fields = ParsePrimaryOrderAudit(body)
	case KindWSSubscriptions:
		fields = ParsePrimaryWSState(body)
	case KindInstrumentCache:
		fields = ParsePrimaryInstrumentCache(body)
	case KindStrategyScript:
		fields = ParsePrimaryStrategy(body)
	case KindBacktestHistory, KindInstaller, KindOther, KindUnknown:
		return
	}
	...
	if row.AccountCuentaSuffix4 == "" && fields.AccountID != "" {
		id := fields.AccountID
		if len(id) > 4 {
			id = id[len(id)-4:]
		}
		row.AccountCuentaSuffix4 = id
	}
	...
	row.OrderPerMinuteMax = fields.OrderPerMinMax
	row.InstrumentCount = fields.InstrumentCount
	row.WSSubscriptionCount = fields.WSSubCount
	if fields.MaxNotionalCents > 0 {
		row.MaxOrderNotionalARSCents = fields.MaxNotionalCents
	}
}
