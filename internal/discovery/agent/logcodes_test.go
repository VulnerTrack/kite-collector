...
// TestLogCodes_FollowConvention pins the `agent_discovery.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeProbeInterfacesCollectFailed,
		LogCodeProbeSoftwareCollectFailed,
		LogCodeProbeDriversCollectFailed,
		LogCodeProbeDriverCollectorErrors,
		LogCodeProbeInterfaceAddrsFailed,
		LogCodeProbeSoftwareParseErrors,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "agent_discovery", parts[0],
				"code %q must lead with the agent_discovery namespace prefix", s)
