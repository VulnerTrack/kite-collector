...
// entry the agent editorext discovery package emits. Convention:
// `agent_editorext.<surface>.<event>` so downstream tooling (Loki/Splunk
...
const (
	...
	LogCodeChainSourceCollectorFailed LogCode = "agent_editorext.chain.source_collector_failed"
	LogCodeChainCapReached            LogCode = "agent_editorext.chain.cap_reached"
)
