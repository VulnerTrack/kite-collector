...
// fileCollector walks PrismaWeb install roots + per-user dirs.
...
func (c *fileCollector) Name() string { return "winargprismaweb" }

func (c *fileCollector) Collect(_ context.Context) ([]Row, error) {
	out := make([]Row, 0, 16)

	roots := append([]string{}, c.installRoots...)
	if p := strings.TrimSpace(c.getenv("PRISMAWEB_DIR")); p != "" {
	...
	for _, base := range c.usersBases {
		...
		for _, e := range entries {
			...
			for _, rel := range UserPrismaWebDirs() {
...
func (c *fileCollector) mergeFields(row *Row, body []byte) {
	var fields PrismaWebFields
	switch row.ArtifactKind {
	case KindCredentials, KindConfig:
		fields = ParsePrismaWebCredentials(body)
	case KindDailySettlement:
		fields = ParsePrismaWebDailySettlement(body)
	case KindCollateral:
		fields = ParsePrismaWebCollateral(body)
	case KindMarginCalls:
		fields = ParsePrismaWebMarginCalls(body)
	case KindOptionsExercise:
		fields = ParsePrismaWebOptionsExercise(body)
	case KindFCICashflow:
		fields = ParsePrismaWebFCICashflow(body)
	case KindFIXDropCopy:
		fields = ParsePrismaWebFIXDropCopy(body)
	case KindMemberPosition:
		fields = ParsePrismaWebMemberPosition(body)
	case KindInstaller, KindOther, KindUnknown:
		return
	}

	if fields.MemberID != "" {
		row.MemberID = fields.MemberID
	}
	...
	if fields.MarginCallCount > 0 {
		row.MarginCallCount = fields.MarginCallCount
	}
	if fields.OptionsExerciseCount > 0 {
		row.OptionsExerciseCount = fields.OptionsExerciseCount
	}
	if fields.CEDEARSettlementCount > 0 {
		row.CEDEARSettlementCount = fields.CEDEARSettlementCount
	}
	if fields.FCICashflowCount > 0 {
		row.FCICashflowCount = fields.FCICashflowCount
	}
	if fields.CollateralCents > 0 {
		row.CollateralARSCents = fields.CollateralCents
	}
...
func classifyAccount(k ArtifactKind, f PrismaWebFields) AccountClass {
	if f.HasFIXDropCopy {
		return AccountALYCClearing
	}
	if k == KindFCICashflow || f.FCICashflowCount > 0 {
		return AccountFCIManager
	}
	if f.CollateralCents > 0 || f.MarginCallCount > 0 {
		return AccountALYCClearing
	}
	if f.SettlementCount > 0 {
		return AccountALYCNonClearing
	}
