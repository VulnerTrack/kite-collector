...
// fileCollector walks HomeBroker install roots + per-user dirs.
...
func (c *fileCollector) Name() string { return "winarghomebroker" }

func (c *fileCollector) Collect(_ context.Context) ([]Row, error) {
	out := make([]Row, 0, 16)

	roots := append([]string{}, c.installRoots...)
	if p := strings.TrimSpace(c.getenv("HOMEBROKER_DIR")); p != "" {
	...
	for _, base := range c.usersBases {
		...
		for _, e := range entries {
			...
			for _, rel := range UserHomeBrokerDirs() {
...
func (c *fileCollector) mergeFields(row *Row, body []byte, name string) {
	...
	case KindCredentials, KindConfig:
		fields = ParseHBCredentials(body, name)
	case KindWatchlist:
		fields = ParseHBWatchlist(body, name)
	case KindPositionsCache:
		fields = ParseHBPositions(body, name)
	case KindOrdersCache:
		fields = ParseHBOrders(body, name)
	case KindSignalRLog:
		fields = ParseHBSignalRLog(body, name)
	case KindSkin:
		fields = ParseHBSkin(body, name)
	case KindChartTemplate, KindInstaller, KindOther, KindUnknown:
		return
	}

	if fields.ALYCBranding != "" {
		row.ALYCBranding = fields.ALYCBranding
	}
	if fields.BrokerMatricula != "" {
		row.BrokerMatricula = fields.BrokerMatricula
	}
	if fields.SignalRToken != "" {
		row.HasSignalRToken = true
		row.SignalRTokenHash = HashSecret(fields.SignalRToken)
	}
	if fields.Username != "" {
	...
	if fields.OrderEventCount > 0 {
		row.OrderEventCount = fields.OrderEventCount
	}
	if fields.CancelEventCount > 0 {
		row.CancelEventCount = fields.CancelEventCount
	}
	if fields.FillEventCount > 0 {
		row.FillEventCount = fields.FillEventCount
	}
	row.CancelRateBps = CancelRateBps(row.OrderEventCount,
		row.CancelEventCount, row.FillEventCount)
	row.AccountClass = classifyAccount(*row, fields)
}

// classifyAccount picks an account-class from parsed fields.
func classifyAccount(row Row, f HBFields) AccountClass {
	if f.SignalRToken != "" && row.ArtifactKind == KindSignalRLog {
		return AccountAPIScraper
	}
	if row.ArtifactKind == KindOrdersCache && row.OrderEventCount > 100 {
		return AccountAPIScraper
	}
	if f.Username != "" {
