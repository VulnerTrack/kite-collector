...
// fileCollector walks Perfil install roots + per-user dirs.
...
func (c *fileCollector) Name() string { return "winargperfilinversor" }

func (c *fileCollector) Collect(_ context.Context) ([]Row, error) {
	out := make([]Row, 0, 16)

	roots := append([]string{}, c.installRoots...)
	if p := strings.TrimSpace(c.getenv("PERFIL_INVERSOR_DIR")); p != "" {
	...
	for _, base := range c.usersBases {
		...
		for _, e := range entries {
			...
			for _, rel := range UserPerfilDirs() {
...
func (c *fileCollector) consider(path, user string, out *[]Row) {
	...
	row := Row{
		...
		AgenteClass:  AgenteClassFromPath(path),
		RiskCategory: CategoryUnknown,
		PeriodYYYYMM: PeriodFromFilename(filepath.Base(path)),
	}
	if row.AgenteClass == AgenteUnknown {
		row.AgenteClass = AgenteOther
	}
	...
	// PDF / DOCX / XLSX are binary — hash only.
	...
	if IsProfileOverdue(row.LastReviewDate, c.now()) {
		row.HasOutdatedProfile = true
	}
...
func (c *fileCollector) mergeFields(row *Row, body []byte) {
	...
	fields := ParsePerfilArtifact(body)
	...
	if fields.RiskCategory != CategoryUnknown {
		row.RiskCategory = fields.RiskCategory
	}
	...
	if row.InstrumentClassList == "" && fields.InstrumentClassList != "" {
		row.InstrumentClassList = fields.InstrumentClassList
	}
	if fields.DeclaredAnnualIncome > 0 {
		row.DeclaredAnnualIncomeCents = fields.DeclaredAnnualIncome
	}
	if fields.DeclaredNetWorth > 0 {
		row.DeclaredNetWorthCents = fields.DeclaredNetWorth
	}
	if fields.HasMissingSignature {
		row.HasMissingSignature = true
	}
	if fields.HasNoKYCLink {
		row.HasNoKYCLink = true
	}
	if fields.HasAggressiveNoTest {
		row.HasAggressiveNoTest = true
	}
}
