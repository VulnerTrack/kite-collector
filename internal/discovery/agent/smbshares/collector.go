...
// fileCollector reads /etc/samba/smb.conf + every drop-in. Samba's
// own `Include = ...` directive is not followed here — operators
// typically use the `/etc/samba/smb.conf.d/` drop-in directory
// instead, and following arbitrary includes is path-traversal-prone.
...
// NewCollector returns the default smb.conf walker.
func NewCollector() Collector {
	return &fileCollector{
		mainFile:  "/etc/samba/smb.conf",
		dropInDir: "/etc/samba/smb.conf.d",
...
func (c *fileCollector) Name() string { return "smb-shares-files" }

func (c *fileCollector) Collect(ctx context.Context) ([]Share, error) {
...
// in `dir` ending in `suffix`. Backup/swap variants are filtered.
