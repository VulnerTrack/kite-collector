// Package winargmotivewave audits MotiveWave artifact files
...
// specialized in:
//
//  1. Automatic Elliott Wave detection (flagship niche).
...
//   - `has_java_algo_strategy=1` — .java algo source.
//   - `has_elliott_wave_rules=1` — Elliott Wave auto-rule.
//   - `has_third_party_extension=1` — .zip Study pack.
...
// ArtifactKind pinned to host_arg_motivewave.artifact_kind.
type ArtifactKind string

const (
	KindConfig           ArtifactKind = "motivewave-config"
	KindCredentials      ArtifactKind = "motivewave-credentials"
	KindWorkspace        ArtifactKind = "motivewave-workspace"
	KindTemplate         ArtifactKind = "motivewave-template"
	KindJavaStrategy     ArtifactKind = "motivewave-java-strategy"
	KindClassCompiled    ArtifactKind = "motivewave-class-compiled"
	KindExtensionPack    ArtifactKind = "motivewave-extension-pack"
	KindConnectionConfig ArtifactKind = "motivewave-connection-config"
	KindDOMConfig        ArtifactKind = "motivewave-dom-config"
	KindSessionLog       ArtifactKind = "motivewave-session-log"
	KindInstaller        ArtifactKind = "motivewave-installer"
	KindOther            ArtifactKind = "other"
	KindUnknown          ArtifactKind = "unknown"
)

// AccountClass pinned to host_arg_motivewave.account_class.
type AccountClass string

const (
	AccountTechnicalAnalyst   AccountClass = "technical-analyst"
	AccountElliottWaveTrader  AccountClass = "elliott-wave-trader"
...
// ProductClass pinned to host_arg_motivewave.product_class.
...
// BrokerPlugin pinned to host_arg_motivewave.broker_plugin.
...
const (
	...
	PluginTradeStation BrokerPlugin = "tradestation"
	PluginTradeKing    BrokerPlugin = "tradeking"
	PluginCustom       BrokerPlugin = "custom"
	PluginNone         BrokerPlugin = "none"
	PluginUnknown      BrokerPlugin = "unknown"
)

// Row mirrors host_arg_motivewave column shape.
type Row struct {
	...
	MotiveWaveAccountID        string       `json:"motivewave_account_id,omitempty"`
	...
	ElliottWaveRuleCount       int64        `json:"elliott_wave_rule_count,omitempty"`
	...
	HasJavaAlgoStrategy        bool         `json:"has_java_algo_strategy"`
	HasElliottWaveRules        bool         `json:"has_elliott_wave_rules"`
	HasThirdPartyExtension     bool         `json:"has_third_party_extension"`
...
// DefaultInstallRoots is the curated MotiveWave install-roots.
func DefaultInstallRoots() []string {
	return []string{
		`C:\Program Files\MotiveWave`,
		`C:\Program Files (x86)\MotiveWave`,
		`C:\MotiveWave`,
		"/opt/MotiveWave",
		"/opt/motivewave",
		"/Applications/MotiveWave.app",
	}
}
...
// UserMotiveWaveDirs is the curated per-user relative paths.
func UserMotiveWaveDirs() [][]string {
	return [][]string{
		{"MotiveWave"},
		{"MotiveWave-extensions"},
		{"Documents", "MotiveWave"},
		{"AppData", "Roaming", "MotiveWave"},
		{"AppData", "Local", "MotiveWave"},
		{".motivewave"},
		{".config", "motivewave"},
		{"projects", "motivewave"},
		{"Library", "Application Support", "MotiveWave"},
		{"Descargas"},
		{"Downloads"},
	}
}
...
func IsCandidateName(name string) bool {
	...
	for _, tok := range []string{
		"motivewave", "motive_wave", "motive-wave",
		"mw_strategy", "mw-strategy",
		"mw_extension", "mw-extension",
		"elliott_wave", "elliott-wave", "elliottwave",
		"dom_config", "dom-config",
		"trade_replay", "trade-replay",
		"strategy_sdk", "strategy-sdk",
	} {
		if strings.Contains(n, tok) {
			return true
		}
	}
	// .java / .class / .jar / .zip only in motivewave context.
	if (ext == ".java" || ext == ".class" || ext == ".jar" || ext == ".zip") &&
		(strings.Contains(n, "motivewave") ||
			strings.Contains(n, "strategy") ||
			strings.Contains(n, "indicator") ||
			strings.Contains(n, "study")) {
		return true
	}
	return false
}
...
func ArtifactKindFromName(name string) ArtifactKind {
	...
	case ".msi", ".exe", ".pkg", ".dmg":
		if strings.Contains(n, "motivewave") {
	...
	case ".mwt":
		return KindTemplate
	case ".java":
		if strings.Contains(n, "strategy") || strings.Contains(n, "motivewave") ||
			strings.Contains(n, "indicator") {
			return KindJavaStrategy
		}
		return KindOther
	case ".class":
		if strings.Contains(n, "strategy") || strings.Contains(n, "motivewave") ||
			strings.Contains(n, "indicator") {
			return KindClassCompiled
		}
		return KindOther
	case ".zip", ".jar":
		if strings.Contains(n, "motivewave") ||
			strings.Contains(n, "study") ||
			strings.Contains(n, "extension") ||
			strings.Contains(n, "mw_extension") ||
			strings.Contains(n, "mw-extension") {
			return KindExtensionPack
		}
		return KindOther
	}
	switch {
	...
	case strings.Contains(n, "motivewave") &&
		(ext == ".log" || ext == ".txt"):
...
func IsCredentialKind(k ArtifactKind) bool {
	switch k {
	case KindConfig, KindCredentials, KindWorkspace, KindTemplate,
...
func AnnotateSecurity(r *Row) {
	...
	if r.ArtifactKind == KindJavaStrategy ||
		r.ArtifactKind == KindClassCompiled ||
		r.StrategyCount > 0 {
		r.HasJavaAlgoStrategy = true
	}
	if r.ArtifactKind == KindExtensionPack {
		r.HasThirdPartyExtension = true
	}
	if r.ArtifactKind == KindConnectionConfig {
		r.HasBrokerPluginCredentials = true
	}
	if r.ElliottWaveRuleCount > 0 {
		r.HasElliottWaveRules = true
	}
