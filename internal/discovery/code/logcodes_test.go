...
// TestLogCodes_FollowConvention pins the `code.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeScannerPathResolveFailed,
		LogCodeScannerWalkFailed,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "code", parts[0],
				"code %q must lead with the code namespace prefix", s)
