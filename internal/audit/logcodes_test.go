...
// TestLogCodes_FollowConvention pins the `audit.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeAuditorFailed,
		LogCodePermissionsStatFailed,
		LogCodeServicePortDiscoveryFailed,
		LogCodeSSHPermissionDenied,
		LogCodeContainerEnvMissingTag,
		LogCodeContainerEnvListFailed,
	...
	for _, c := range codes {
		...
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "audit", parts[0],
				"code %q must lead with the audit namespace prefix", s)
