...
// ldapMachineFromTags builds an LDAP-discovered machine with the supplied tag
// map encoded as JSON. Hostname/MachineID are deterministic enough for
// tests to assert against the returned findings without flakiness.
func ldapMachineFromTags(t *testing.T, tags map[string]any) model.Machine {
	t.Helper()
...
func TestLDAP_Audit_SkipsNonLDAPMachine(t *testing.T) {
	a := newLDAPForTest(LDAPAuditConfig{})
	machine := model.Machine{ID: uuid.New(), DiscoverySource: "agent"}
	got, err := a.Audit(context.Background(), machine)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(got) != 0 {
		t.Errorf("non-ldap machine returned %d findings", len(got))
	}
}
...
func TestLDAP_Audit_StaleAccount(t *testing.T) {
	...
	machine := ldapMachineFromTags(t, map[string]any{
	...
	got, err := newLDAPForTest(LDAPAuditConfig{StaleThresholdDays: 90}).Audit(context.Background(), machine)
	if err != nil {
...
func TestLDAP_Audit_StaleAccount_NeverLoggedSkipped(t *testing.T) {
	machine := ldapMachineFromTags(t, map[string]any{contract.AttrADEnabled: true})
	got, err := newLDAPForTest(LDAPAuditConfig{StaleThresholdDays: 90}).Audit(context.Background(), machine)
	if err != nil {
		t.Fatalf("Audit: %v", err)
	}
...
func TestLDAP_Audit_KerberoastableSPN(t *testing.T) {
	...
	got, err := newLDAPForTest(LDAPAuditConfig{}).Audit(context.Background(), machine)
	if err != nil {
		t.Fatalf("Audit: %v", err)
	}
...
func TestLDAP_Audit_KerberoastableSPN_OnlyDefaultSPNs(t *testing.T) {
	...
	got, _ := newLDAPForTest(LDAPAuditConfig{}).Audit(context.Background(), machine)
	if hasCheck(got, "ad-002") {
...
func TestLDAP_Audit_CleartextBind(t *testing.T) {
	machine := ldapMachineFromTags(t, map[string]any{contract.AttrADEnabled: true})
	got, _ := newLDAPForTest(LDAPAuditConfig{TLSMode: "none"}).Audit(context.Background(), machine)
...
func TestLDAP_Audit_CleartextBind_LDAPSSkipped(t *testing.T) {
	machine := ldapMachineFromTags(t, map[string]any{contract.AttrADEnabled: true})
	got, _ := newLDAPForTest(LDAPAuditConfig{TLSMode: "ldaps"}).Audit(context.Background(), machine)
	if hasCheck(got, "ad-004") {
...
func TestLDAP_Audit_AllFourFindings(t *testing.T) {
	...
	got, _ := newLDAPForTest(LDAPAuditConfig{
		...
		TLSMode:            "none",
	}).Audit(context.Background(), machine)

	wantSet := map[string]bool{"ad-001": false, "ad-003": false, "ad-004": false}
	// note: ad-002 needs Enabled=true; this machine is disabled, so we test 1+3+4
	for _, f := range got {
...
func TestLDAP_Audit_MalformedTagsJSON(t *testing.T) {
	...
	_, err := newLDAPForTest(LDAPAuditConfig{}).Audit(context.Background(), machine)
	if err == nil {
...
func TestLDAP_Audit_EmptyTagsNoFindings(t *testing.T) {
	machine := model.Machine{ID: uuid.New(), DiscoverySource: "ldap", Tags: ""}
	got, err := newLDAPForTest(LDAPAuditConfig{}).Audit(context.Background(), machine)
