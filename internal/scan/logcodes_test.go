...
// TestLogCodes_FollowConvention pins the `scan.<surface>.<event>` shape.
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeScanEngineError,
		LogCodeScanEngineStorageExhausted,
		LogCodeScanSubscriberDropped,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "scan", parts[0],
				"code %q must lead with the scan namespace prefix", s)
