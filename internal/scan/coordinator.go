...
func (c *Coordinator) runScan(scanCtx context.Context, cancel context.CancelFunc, done chan struct{}, scanID uuid.UUID, req StartRequest) {
	...
	if err != nil {
		...
		if errors.Is(err, sqlite.ErrTransientStorageExhausted) {
			c.logger.Warn("scan engine returned storage-exhausted error; remediation is known",
				"code", string(LogCodeScanEngineStorageExhausted),
				"scan_id", scanID,
				"error", err,
				"status", status,
			...
			c.logger.Warn("scan engine returned error",
				"code", string(LogCodeScanEngineError),
				"scan_id", scanID,
				"error", err,
				"status", status)
		}
...
func (c *Coordinator) publish(ev Event) {
	...
	for id, ch := range c.subs {
		...
		case ch <- ev:
		default:
			c.logger.Debug("scan-event subscriber buffer full; event dropped",
				"code", string(LogCodeScanSubscriberDropped),
				"subscriber", id,
				"scan_id", ev.ScanRunID,
				"type", ev.Type,
				"subscriber_count", len(c.subs))
		}
	}
}
