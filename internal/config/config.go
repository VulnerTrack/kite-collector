...
// heartbeat reconciliation, canary baselines, and tamper detection.
type ObservabilityConfig struct {
	Canary CanaryConfig `mapstructure:"canary"`
}

// CanaryConfig declares the set of discovery sources expected to emit a
// heartbeat in every scan on this host. After each scan the reconciler
// diffs the actual heartbeats against this list and raises an
// IncidentCanaryDrift for any missing collector.
//
// When ExpectedCollectors is empty, drift checks are disabled — useful
// for fleets where the baseline is set centrally and pushed later. When
// Strict is true, extra collectors (heartbeats from sources not in the
// expected list) also raise drift; otherwise extras are accepted silently.
type CanaryConfig struct {
	ExpectedCollectors []string `mapstructure:"expected_collectors"`
