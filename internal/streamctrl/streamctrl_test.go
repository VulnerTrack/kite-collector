...
func (r *recordingEmitter) Emit(_ context.Context, _ model.MachineEvent) error {
	r.single++
	return r.err
}

func (r *recordingEmitter) EmitBatch(_ context.Context, events []model.MachineEvent) error {
	r.batch += len(events)
	return r.err
}
...
func TestController_DefaultIdle_DropsEmits(t *testing.T) {
	...
	require.NoError(t, c.Emit(context.Background(), model.MachineEvent{}))
	require.NoError(t, c.EmitBatch(context.Background(), []model.MachineEvent{{}, {}}))
...
func TestController_EmitErrorMarksDegraded(t *testing.T) {
	...
	c := New(rec)
	require.NoError(t, c.Start(context.Background()))
	_ = c.Emit(context.Background(), model.MachineEvent{})
	status := c.Status()
