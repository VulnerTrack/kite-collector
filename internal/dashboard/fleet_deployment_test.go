...
func TestFleetMachinesFromLatestDiscovery_UnfilteredBeforeFirstScan(t *testing.T) {
	// No discovery session yet (nil keys): the page promises the local
	// Machines inventory, so an empty key set means "no narrowing", not
	// "hide everything" (see fleetMachinesFromLatestDiscovery).
...
func TestFleetDeployScript_WindowsCredentialsAreAlwaysRequired(t *testing.T) {
	...
	assert.Contains(t, script, `KITE_WINDOWS_LOCAL_USER="${KITE_WINDOWS_USER##*\\}"`)
	assert.Contains(t, script, "This Windows account has no password")
	assert.Contains(t, script, `net.exe user "%s" *`)
	assert.Contains(t, script, "Then return to this terminal")
...
func TestRoute_POST_FleetPackage_ReturnsZip(t *testing.T) {
	...
	assert.Contains(t, bundleFiles, "regenerate-package.sh")
	...
	assert.Contains(t, bundleFiles["playbooks/deploy.yml"], "Normalize modern Windows service command line")
	assert.Contains(t, bundleFiles["playbooks/deploy.yml"], `Invoke-CimMethod -InputObject $service -MethodName Change`)
	assert.NotContains(t, bundleFiles["playbooks/deploy.yml"], `sc.exe config kite-collector binPath=`)
	assert.Contains(t, bundleFiles["playbooks/deploy.yml"], "Read installed collector version")
	assert.Contains(t, bundleFiles["playbooks/deploy.yml"], "kite_install_required")
	...
	assert.Contains(t, bundleFiles["preflight.py"], "return to the Fleet deployment screen")
	assert.Contains(t, bundleFiles["preflight.py"], "click Generate deployment package")
	assert.Contains(t, bundleFiles["preflight.py"], "do not reuse this package")
	assert.Contains(t, bundleFiles["preflight.py"], "./regenerate-package.sh")
	assert.NotContains(t, bundleFiles["preflight.py"], "Then rerun ./deploy.sh")
	assert.Contains(t, bundleFiles["regenerate-package.sh"], `dashboard_url="${KITE_DASHBOARD_URL:-http://127.0.0.1:9090}"`)
	assert.Contains(t, bundleFiles["regenerate-package.sh"], `pc-001.example.test,windows,amd64,`)
	assert.Contains(t, bundleFiles["regenerate-package.sh"], `/api/v1/fleet/package`)
	assert.Contains(t, bundleFiles["regenerate-package.sh"], `zipfile.ZipFile`)
	assert.Contains(t, bundleFiles["README.md"], `run ./regenerate-package.sh`)
	assert.Contains(t, bundleFiles["README.md"], `do not reuse the failed package`)
