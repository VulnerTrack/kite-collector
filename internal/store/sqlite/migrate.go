...
func (s *SQLiteStore) applyMigrations(ctx context.Context) error {
	...
	for version, stored := range applied {
		...
		if stored.checksum != current {
			slog.Error(
				...
				"file_name", fileName,
				"hint", "either restore the file to its applied content or use RepairMigration if the edit was deliberate",
			)
		}
	}
	...
	for _, file := range files {
		...
		slog.Info("applying migration",
			"code", string(LogCodeMigrateApplying),
			"version", version,
			"checksum", checksum[:12],
			"sql_bytes", len(sqlBytes))
...
func (s *SQLiteStore) RepairMigration(ctx context.Context, version string) error {
	...
	slog.Info("migration entry removed from schema_migrations; will re-apply on next Migrate() call",
		"code", string(LogCodeMigrateEntryRemoved),
		"version", version,
		"rows_affected", n)
	return nil
}
