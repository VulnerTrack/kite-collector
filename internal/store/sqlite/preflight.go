...
func WarnIfPathIsSuspect(logger *slog.Logger, dbPath string) {
	...
	for _, frag := range suspectPathFragments {
		if strings.Contains(lower, frag) {
			logger.Warn("db_path is on a sync/network volume; SQLite needs exclusive local FS access",
				"code", string(LogCodePreflightSyncVolume),
				"db_path", dbPath,
	...
	if runtime.GOOS == "windows" && (strings.HasPrefix(dbPath, `\\`) || strings.HasPrefix(dbPath, `//`)) {
		logger.Warn("db_path is on a UNC/SMB share; SQLite needs exclusive local FS access",
			"code", string(LogCodePreflightUNCShare),
			"db_path", dbPath,
	...
	if fsType, mountPoint, ok := lookupMountFSType(resolved); ok {
		if hint, bad := suspectFSTypes[strings.ToLower(fsType)]; bad {
			logger.Warn("db_path is on a non-local filesystem; SQLite needs exclusive local FS access",
				"code", string(LogCodePreflightNonLocalFS),
				"db_path", dbPath,
				"resolved_path", resolved,
