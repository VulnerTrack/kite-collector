...
func TestLogSoftwareParseErrors_LogsEachError(t *testing.T) {
	...
	// LogCode pattern: "code" is the stable identifier alert rules
	// pivot on. Message text is informational and may evolve.
	assert.Equal(t, string(LogCodeSoftwareParseError), records[0]["code"],
		"each parse-error log must carry the engine.software.parse_error code so downstream alerts can pivot")
...
func TestLogSoftwareParseErrors_TruncatesAfterCap(t *testing.T) {
	...
	assert.Equal(t, string(LogCodeSoftwareParseTruncated), last["code"],
		"truncation log must carry engine.software.parse_errors_truncated so alerts can fire on flood events")
