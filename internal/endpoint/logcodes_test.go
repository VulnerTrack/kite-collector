...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeManagerConnectSkipped,
		LogCodeManagerNoTLS,
		LogCodeHealthDegraded,
		LogCodeHealthUnreachable,
		LogCodeTOFUMismatch,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "endpoint", parts[0],
				"code %q must lead with the endpoint namespace prefix", s)
