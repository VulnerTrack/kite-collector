...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeManagerStartUnhealthy,
		LogCodeManagerRestartLimit,
		LogCodeManagerSubprocessExited,
		LogCodeManagerRestartFailed,
		LogCodeManagerRestartUnhealthy,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "tunnel", parts[0],
				"code %q must lead with the tunnel namespace prefix", s)
