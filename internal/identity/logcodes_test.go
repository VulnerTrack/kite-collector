...
// TestLogCodes_FollowConvention pins the `identity.<surface>.<event>`
...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeDetectTPMUnavailable,
		LogCodeDetectKeyringUnavailable,
		LogCodeHardenDumpableFailed,
		LogCodeHardenMlockallFailed,
		LogCodeLifecycleStampExpectedHashFailed,
		LogCodeLifecycleBinaryHashUnavailable,
		LogCodeLifecycleExpectedHashUnavailable,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "identity", parts[0],
				"code %q must lead with the identity namespace prefix", s)
