...
const (
	...
	// IncidentTamperDetected fires when a probe heartbeat has an invalid
	// Ed25519 signature, or the running binary's SHA-256 diverges from
	// identity.ExpectedBinaryHash. Both cases indicate the agent binary or
	// identity material was replaced between launches.
	IncidentTamperDetected IncidentType = "tamper_detected"
	// IncidentCanaryDrift fires when the set of collectors that emitted a
	// heartbeat for a scan diverges from config.Observability.Canary
	// .ExpectedCollectors. A collector that silently stopped reporting is
	// the most common cause.
	IncidentCanaryDrift IncidentType = "canary_drift"
)
