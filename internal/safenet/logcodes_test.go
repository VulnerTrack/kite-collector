...
func TestLogCodes_FollowConvention(t *testing.T) {
	codes := []LogCode{
		LogCodeTLSVerificationDisabled,
		LogCodeEnvInvalidBool,
		LogCodeEnvInvalidInt,
		LogCodeEnvGoroutinePanic,
		LogCodeScopeGuardConcurrencyClamped,
		LogCodeScopeGuardInvalidIntEnv,
	}
	for _, c := range codes {
		s := string(c)
		t.Run(s, func(t *testing.T) {
			...
			assert.Equal(t, "safenet", parts[0],
				"code %q must lead with the safenet namespace prefix", s)
